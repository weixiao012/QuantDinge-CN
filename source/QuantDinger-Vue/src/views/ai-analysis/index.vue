    ...
    async saveMonitorTask () {
      ...
      try {
        const res = await addMonitor({
          ...
          monitor_type: 'ai',
          config: { run_interval_minutes: interval, symbol: stock.symbol, market: stock.market, language: this.$store.getters.lang || this.$i18n.locale || 'zh-CN' },
      ...
      for (const key of keys) {
        ...
        try {
          await addMonitor({
            ...
            position_ids: positionIds,
            monitor_type: 'ai',
            config: { run_interval_minutes: interval, symbol, market, language: this.$store.getters.lang || this.$i18n.locale || 'zh-CN' },
            notification_config: { channels: notifyChannels },
            is_active: true
          })
          created++
