...
export default {
  ...
  methods: {
    ...
    async loadAiSkills () {
      ...
      try {
        const res = await getAiSkills({ language: (this.$i18n && this.$i18n.locale) || 'zh-CN' })
        const data = res.data || {}
    ...
    async loadCalendar (force = false) {
      ...
      this.calendarError = ''
      try {
        const res = await getEconomicCalendar({ force: force ? 1 : 0, days: 14, lang: (this.$i18n && this.$i18n.locale) || 'zh-CN' })
        const data = res.data || {}
